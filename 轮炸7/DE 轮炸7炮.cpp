...
void ShovelCheck() {
    ...
    for (auto& Plant : aAlivePlantFilter)
        if (Plant.Type() == ACOB_CANNON)
            ++CobCount;
    if (CobCount < 7 || RealSeedCD(ACOB_CANNON) != 0)
        return;

    int MinHp = 200;
    int MinHpRow = 0;
    int MinHpCol = 0;
    for (auto [Row, Col] : std::vector<std::pair<int, int>> {{1, 5}, {2, 5}, {3, 5}, {4, 5}, {5, 5}, {2, 1}, {4, 1}}) {
...
void Logic() {
    ...
    if (RealSeedCD(AGLOOM_SHROOM) == 0) {
        ...
        for (auto [Row, Col] : std::vector<std::pair<int, int>> {{3, 7}, {1, 1}, {3, 1}, {5, 1}}) {
    ...
    if (AGetMainObject()->GameClock() % 994 == 0)
        aCobManager.Fire(2, 8.6);
    if (AGetMainObject()->GameClock() % 994 == 497)
        aCobManager.Fire(4, 8.6);
    ...
    if (FixPumpkin && AIsSeedUsable(APUMPKIN) && !Check_Zombie(AZOMBONI, 0, 3, -1000, 591)) {
        if (AGetPlantIndex(3, 7, APUMPKIN) >= 0) {
            if (AGetPlantPtr(3, 7, APUMPKIN)->Hp() < 300)
                Use_Card(APUMPKIN, 3, 7);
        } else {
            Use_Card(APUMPKIN, 3, 7);
        }
    }
    ...
    for (auto* Zombie : ZombieCache) {
        if (Zombie->Type() == ABALLOON_ZOMBIE && int(Zombie->Abscissa() - BalloonΔX(851, Zombie->Speed(), Zombie->SlowCountdown())) <= -100) {
            if (!Check_Plant(ABLOVER, 2, 7))
                ARemovePlant(2, 7);
            Use_Card(ABLOVER, 2, 7);
        }
    }
    for (int Row : {3})
        if (Check_Zombie(AGIGA_GARGANTUAR, 0, Row, -1000, 630))
    ...
    for (int Row : {1, 5})
        if (Check_Zombie(APOLE_VAULTING_ZOMBIE, 11, Row, -1000, 685))
            ARemovePlant(Row, 8);
        else if (Check_Zombie(AFOOTBALL_ZOMBIE, 0, Row, 560, 600) || Check_Zombie(AGIGA_GARGANTUAR, 0, Row, 600, 680) || Check_Zombie(AJACK_IN_THE_BOX_ZOMBIE, 15, Row, 580, 630) || Check_Zombie(ALADDER_ZOMBIE, 76, Row, 580, 640))
    ...
    if (Check_Zombie(AFOOTBALL_ZOMBIE, 0, 1, -1000, 450) || Check_Zombie(AFOOTBALL_ZOMBIE, 0, 2, -1000, 450) || Check_Zombie(AZOMBONI, 0, 1, -1000, 485) || Check_Zombie(AGIGA_GARGANTUAR, 0, 1, -1000, 511)
        ...
        if (!Check_Plant(ABLOVER, 2, 7) && !Check_Plant(ACHERRY_BOMB, 2, 7))
            ARemovePlant(2, 7);
        Use_Card(ACHERRY_BOMB, 2, 7);
    }

    if (Check_Zombie(AFOOTBALL_ZOMBIE, 0, 4, -1000, 450) || Check_Zombie(AFOOTBALL_ZOMBIE, 0, 5, -1000, 450) || Check_Zombie(AZOMBONI, 0, 5, -1000, 485) || Check_Zombie(AGIGA_GARGANTUAR, 0, 5, -1000, 511)
        || Check_Zombie(ALADDER_ZOMBIE, 76, 5, -1000, 480) || Check_Zombie(AJACK_IN_THE_BOX_ZOMBIE, 15, 5, -1000, 470)) {
        if (!Check_Plant(ABLOVER, 4, 7) && !Check_Plant(ACHERRY_BOMB, 4, 7))
            ARemovePlant(4, 7);
...
void AScript() {
    ...
    aPlantFixer.Start(APUMPKIN, {{1, 1}, {3, 1}, {5, 1}}, 1000);
