...
void Logic() {
    ...
    if (RealSeedCD(AGLOOM_SHROOM) == 0) {
        ...
        for (auto [Row, Col] : std::vector<std::pair<int, int>> {{3, 8}, {3, 7}, {2, 7}, {4, 7}, {1, 1}, {3, 1}, {5, 1}}) {
    ...
    for (int Row : {3}) {
        if (FixPumpkin && AIsSeedUsable(APUMPKIN) && !Check_Zombie(AZOMBONI, 0, Row, -1000, 671) && !Check_Zombie(ACATAPULT_ZOMBIE, 0, Row, -1000, 671)) {
            if (AGetPlantIndex(Row, 8, APUMPKIN) >= 0) {
                if (AGetPlantPtr(Row, 8, APUMPKIN)->Hp() < 1000)
                    Use_Card(APUMPKIN, Row, 8);
            } else {
                Use_Card(APUMPKIN, Row, 8);
            }
        }
    ...
    for (int Row : {3})
        if (Check_Zombie(AGIGA_GARGANTUAR, 0, Row, 680, 760))
            Use_Meatshield(Row, 9);
    ...
    if (Check_Zombie(AFOOTBALL_ZOMBIE, 0, 4, -1000, 450) || Check_Zombie(AFOOTBALL_ZOMBIE, 0, 5, -1000, 450) || Check_Zombie(AZOMBONI, 0, 5, -1000, 485) || Check_Zombie(AGIGA_GARGANTUAR, 0, 5, -1000, 511)
        ...
        if (!Check_Plant(ABLOVER, 4, 8) && !Check_Plant(ACHERRY_BOMB, 4, 8))
            ARemovePlant(4, 8);
        Use_Card(ACHERRY_BOMB, 4, 8);
    }
}
